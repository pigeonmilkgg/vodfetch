...
- [x] **Batch 5 — Converter + Transcript (new-CSV expansion)** (built 2026-07-06, awaiting go-live): a second Ubersuggest set (twitch_converter / twitch_transcript seeds) surfaced two on-mission clusters we didn't cover. Shipped (EN+DE+FR+ES): new landing `/twitch-converter` (targets "twitch converter" 210 + "twitch converter mp4" 170 + "twitch to mp4/mp3 converter" — reframes existing MP4/MP3/GIF output as a converter, same client tool); new guide `/blog/how-to-get-twitch-transcript` (honest answer to the transcript cluster — "twitch transcript" 140, "does twitch have transcript/subtitles", "twitch transcript generator" CPC $1.40–$2.16: chat replay = text transcript we already do, + audio→MP3→speech-to-text for spoken words we plainly don't). DE video landing retargeted to **"Twitch Videos herunterladen"** (9,900) and FR to **"Télécharger des vidéos Twitch"** (native download verbs in title/h1/meta/lead); chat-transcript FAQ added to the chat landing; AEO quick-answers for transcript + converter added to llms.txt/ai.json. Wired into internal-linking maps (0 orphans, 0 broken links); full @graph JSON-LD (HowTo+FAQ) on both pages. **Deliberately SKIPPED as off-mission** (big volume, wrong mission): bits→money calculators, emote/badge/banner size converters, adblock scripts, overlays, streamer-name discovery pages, "Twitch app on Roku/PS5/Xbox", payment-methods, buffering troubleshooting.

**Five batches built** (1–4 live; 5 built locally, awaiting your push to deploy). On-page/AEO is maxed; the remaining lever is off-page (backlinks + GSC request-indexing + time on the new domain).
