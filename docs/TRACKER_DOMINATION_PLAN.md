...
- [x] **T2 — Client-side live tools** (shipped 2026-07-06, live + IndexNow 982 URLs): new landing `/twitch-follower-count` (kind `follower`, own card + `checkFollowers()` — exact live total from the public API with timestamped answer sentence, account age, Partner badge, live viewers; EN+DE+FR+ES). **Honesty finding, verified against the live API:** anonymous followage (who follows / when someone followed) is blocked by Twitch — shipped as an honest FAQ instead of a fake feature; growth-tracking FAQ points to our own since-2026 snapshot collection. Clip search: channel-browser top-clips now filterable by period (Today/Week/Month/All-time — LAST_DAY/WEEK/MONTH criteria verified against production API), on every tool page incl. the clip landing. Build 983 pages (+14), audit 0 errors.
- [x] **T3 — Facts & transparency layer** (shipped 2026-07-06, live + IndexNow 1024 URLs): three new info pages in all 14 languages (EN+DE+FR+ES native, rest EN-fallback): `/twitch-records` (sourced all-time records: Ibai La Velada V 9,334,779 peak 07/2025 + Velada IV/III/II + TheGrefg 2,470,347; Kai Cenat 1M+ active subs 09/2025 + full documented sub-record line Ludwig/Ninja; most-followed top group; every figure dated + sourced, self-committing update promise), `/twitch-sub-counts` (why every live sub number is an estimate — no public API, how trackers infer, followage explanation, what vodfetch refuses to publish), `/methodology` (live lookups vs snapshot archive, known limits, refuse-list, corrections; worded to be true regardless of when Helix collection starts). Info-page renderer generalized: FAQPage JSON-LD + FAQ block for any page with faqs (was editorial-only), stats cluster cross-linking, .md mirrors incl. FAQs. Machine files: quick answers link /twitch-records + /twitch-sub-counts, new "How many subs does Kai Cenat have" honest answer, 3 new _ai_resources URLs. Build 1025 pages (+42), audit 0 errors. **T3.4 /data open datasets DEFERRED** until owner adds Helix secrets — no data pages without real data (honesty rule).
- [x] **T4 — Rankings & German flank** (shipped 2026-07-06, live + IndexNow 1080 URLs): `/most-followed-twitch-streamers` (top 20 with EXACT follower totals measured live from the public API on 2026-07-06 — candidate set from Wikipedia, re-measured channel by channel; ItemList JSON-LD via new ranklist renderer support; honest notes incl. inactive-channel caveat) + `/german-twitch-streamers` (DE-first top 15, exact measured counts — MontanaBlack88 5,759,425 ahead of Trymacs/Papaplatte; **data-quality catch published on-page: 'knossi' is a squatter account, the real one is 'therealknossi'** — measured-vs-copied-list story as trust content) + `/twitch-top-games` (landing kind `topgames`: live top-30 client-rendered from games(sort:VIEWER_COUNT) — verified query — with timestamp + refresh; explicitly NO fake historical averages, methodology cross-link) + blog guide `streamer-communities-twitch-teams` (targets the 40.5K 'streamer communities' keyword; honest Teams-by-invitation/Partner-creation facts). Roster fixed with verified logins (therealknossi, revedtv, fritz_meinecke; +zarbex/rewinside/amar/julien; dropped 2 unresolvable). EN+DE+FR+ES; germanstreamers deliberately EN+DE only (language-matched guardrail). Build 1081 pages (+56), audit 0 errors.
- [x] **T5 — Streamer entity pilot** (shipped 2026-07-06, live + IndexNow 1161 URLs): **80 curated `/streamer/{login}` pages + `/streamer` directory index**, new page type in `_streamers.py` + `render_streamer()`. Language-matched single URLs (41 EN / 20 DE / 9 FR / 10 ES pages, `<html lang>` per streamer, hreflang = self+x-default only — no ×14 multiplication). Shell = measured facts only: exact follower totals + created dates + Partner status queried live from the public API on 2026-07-06, per-page as-of stamps, blurbs = measured facts + hand-curated knowledge lines (~50 channels) + the channel's own public description quoted; zero hallucinated facts. Hydration = the existing channel browser auto-booted via `window.TWDL_BOOT` → recent VODs with expiry badges + period-filtered top clips, every row with MP4 download (the one thing no tracker offers). ProfilePage+Person (Organization for team channels) JSON-LD, per-page FAQPage with unique measured values, .md mirrors, related-streamer cross-links + directory + footer link, sitemap extension. Build 1162 pages (+81), audit 0 errors (87,466 internal edges). **Expansion gate active: add names only after GSC shows >70% of the pilot indexed.**
- [ ] **T6 — "Since 2026" history layer** (gated on T0.4 ≥3–6 months of collected data — earliest ~Q4 2026/Q1 2027; requires owner to add Helix secrets first)
